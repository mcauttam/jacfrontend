...
const MakePaymentReg=(props)=>{
    ...
    const [amount_paid, setAmount] = useState(ReactSession.get("amount_paid"));
    ...
    useEffect(() => {
        ...
        if(ReactSession.get("amount_paid")!=="" || ReactSession.get("amount_paid")!==null || ReactSession.get("amount_paid")!==undefined) {
            setAmount(ReactSession.get("amount_paid"))
        }
        Axios.post(`${process.env.REACT_APP_URI}fee/payu/hash`, {key, txnid, amount_paid, productinfo, firstname, email, salt}).then(res=>{
            setHash(res.data.hash);
        });
    }, [key, txnid, amount_paid, productinfo, firstname, email, salt]);
    ...
    const calcHash =async (e) => {
        const paymentDetails=await Axios.post(`${process.env.REACT_APP_URI}fee/conv/details`,{
            paymentdetail_id:ReactSession.get("txnid"),amount_paid:amount_paid,payment_status:"Pending",
    ...
    return(
        <div className="container text-lg-center mt-4">
            ...
            <Form action={baseURL} method="post" onSubmit={calcHash}>
                ...
                <FormControl type="hidden" name="amount_paid" value={amount_paid} onChange={calcHash}></FormControl>
